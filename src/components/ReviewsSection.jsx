...
function ReviewQuote({ r }) {
  ...
  return (
    <li className="review">
      ...
      <p className="review-by">— {r.name}{r.location ? `, ${r.location}` : ''} · <time dateTime={r.date}>{when}</time></p>
    </li>
  );
}
...
function ReviewForm() {
  ...
  const onSubmit = async ev => {
    ...
    if (Object.keys(e).length) { setTimeout(() => { summaryRef.current?.scrollIntoView({ block: 'start' }); summaryRef.current?.focus({ preventScroll: true }); }, 0); return; }
    setSending(true);
  ...
  return (
    <form className="review-form" noValidate onSubmit={onSubmit}>
      ...
      <Button type="submit" busy={sending}>Send your review</Button>
    </form>
  );
}
